...
do
 ...
 ThemeManager.BuiltInThemes = {
    ...
    function ThemeManager:ThemeUpdate()
        ...
        self.Library.Shader.ImageColor3 = self.Library.Options["AccentColor"].Value
    ...
    function ThemeManager:CreateThemeManager(groupbox)
        ...
        groupbox:AddLabel("次要颜色"):AddColorPicker("AccentColor", { Default = self.Library.Scheme.AccentColor })
        groupbox
            :AddLabel("线条颜色")
        ...
        groupbox:AddButton("加载主题", function()
            ...
            self.Library:Notify(string.format("加载主题为： %q", name))
        end)
        groupbox:AddButton("覆盖主题", function()
            local name = self.Library.Options.ThemeManager_CustomThemeList.Value

            self:SaveCustomTheme(name)
            self.Library:Notify(string.format("覆盖主题为： %q", name))
        end)
        groupbox:AddButton("删除主题", function()
            ...
            self.Library:Notify(string.format("删除主题为： %q", name))
            self.Library.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
            self.Library.Options.ThemeManager_CustomThemeList:SetValue(nil)
        end)
        ...
        groupbox:AddButton("设为默认值", function()
            ...
            then
                ...
                self.Library:Notify(
                    string.format("将默认主题设置为： %q", self.Library.Options.ThemeManager_CustomThemeList.Value)
                )
            end
        end)
        groupbox:AddButton("重置为默认设置", function()
            local success = pcall(delfile, self.Folder .. "/themes/default.txt")
            if not success then
                return self.Library:Notify("无法重置默认设置，删除文件出错")
            end
